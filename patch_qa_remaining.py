...
# Replace LLM logic in full QA
pattern_llm = re.compile(
    r'    # Try the preferred provider first\n'
    r'    if provider:\n'
...
new_llm = """    # Try the preferred provider first
    if provider:
...
pattern_vlm = re.compile(
    ...
    r'.*?'
    r'        else:\n'
    r'            qa_response = attempt_vlm\(provider\)',
    re.DOTALL
)

new_vlm = """    # Try the preferred provider first
    if provider:
