...
def test_is_valid_translation():
    ...
    assert is_valid_translation("こんにちは", "translate the following text: Hello") == False
    
    # Identical to Japanese source check
    assert is_valid_translation("こんにちは", "こんにちは") == False

    # Pathologically long
    assert is_valid_translation("hi", "This is an extremely long translation for a very short text which should definitely fail the validation check because it exceeds the length ratio by a huge margin.") == False
...
def test_should_translate_region():
    ...
    # SFX whitelist
    region_sfx = {"width": 20, "height": 20, "text": "ドン", "confidence": 0.2} # Should pass despite low conf due to whitelist
