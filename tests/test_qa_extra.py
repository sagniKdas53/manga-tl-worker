...
from worker.handlers.qa import process_qa, _auto_pass_all, _process_qa_llm, _process_qa_vlm
...
def test_process_qa_llm_mode(mock_cloud, mock_redis, mock_requests):
    ...
    with patch("worker.handlers.qa.QA_MODE", "llm"):
        with patch("worker.handlers.qa.QA_CONFIG") as mock_qa:
...
def test_process_qa_vlm_mode(mock_dl, mock_cloud_vision, mock_redis, mock_requests):
    ...
    mock_res = MagicMock()
    mock_res.status_code = 200
    mock_res.json.return_value = {"ocrRegions": [{"id": "1", "text": "hello", "translatedText": "hi", "bboxX": 0, "bboxY": 0, "bboxW": 10, "bboxH": 10}]}
    mock_requests.get.return_value = mock_res
    ...
    with patch("worker.handlers.qa.QA_MODE", "vlm"):
        with patch("worker.handlers.qa.QA_CONFIG") as mock_qa:
            mock_qa.provider = "openrouter"
