...
def test_load_font_default_fallback():
    with patch("worker.handlers.render.os.path.exists", return_value=False):
        with patch("worker.handlers.render.ImageFont.truetype", side_effect=Exception("error")):
...
def test_fit_text_in_box_elliptical():
    res = fit_text_in_box_py(
        "hello world", 100, 100, "Comic Neue", shape="elliptical"
    )
...
def test_fit_text_in_box_polygon_complex():
    ...
    res = fit_text_in_box_py(
        "hello world", 100, 100, "Comic Neue", shape="rectangular", mask_polygon=json.dumps(polygon)
    )
    assert res["fontSize"] > 0
